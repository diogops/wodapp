...
- `ensureProgramWorkouts` (also on `workouts.list`, **after** the defaults) seeds the PDF-transcribed programs in `server/programs/` — currently `fortalecimento.ts`, four "Dia N" workouts in the `fortalecimento` modality with a day-only schedule rule each (seg/ter/qui/sex). Idempotency key is `workouts.sourceFileName`, so deleting all four re-seeds them on the next list; schedule rules are only created when the modality has none, so user edits win. `server/programs/programs.test.ts` validates each program against `workoutSchema`, the modality grammar, and the image files on disk.
- `workoutExercises.imageUrl` holds a static path under `client/public/demos/` (the `fortalecimento-*.jpg` files were extracted from the PDF). The client's `getExerciseDemo` prefers it over the name-keyed `EXERCISE_DEMOS` fallback map.
...
The `today` tab applies a `workout-mode` class that locks global scroll (`html:has(.workout-mode)` in `client/src/index.css`) and pins toolbar → card header → scrollable body → action footer inside one viewport. **Only `.workout-card-body` may scroll**; everything else is `flex: 0 0 auto`. On mobile the global app header **stays visible but compact** (app name hidden, selects share the remaining width) because it is the only place to switch modality or open the schedule; the progress counter lives in the session toolbar. `useViewportLock` (`client/src/lib/viewportLock.ts`) resets `window.scrollTo(0,0)` whenever no input is focused: iOS Safari scrolls the locked page to reveal the SetTracker load input and never scrolls it back, which cut the header off. It ignores sub-pixel offsets and has a burst breaker: at device pixel ratio 3 WebKit reports `scrollY` 0.33 after `scrollTo(0,0)`, and without those two guards reset → scroll event → reset loops forever. `.workout-mode` is also `position: fixed; inset: 0` so a residual window offset cannot move the app, and the lock class is applied only after `settled` (two frames after entering Hoje, following a `scrollTo(0,0)` on a still-scrollable page) — see `isWorkoutLocked` in `workoutMode.ts`. Any fixed element wider than the viewport (sonner's toast container was one) lets iOS pan the page sideways; `workout-header.spec.ts` sweeps for that.
